    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="trackViewDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Client')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>TrackView
                </v-btn>
                <v-dialog v-model="trackViewDialog" width="500">
                    <TrackView
                        @closeDialog="trackViewDialog = false"
                        @trackView="trackView"
                    ></TrackView>
                </v-dialog>
            </div>
                ...
                <v-table>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap" label="DraftId">{{ val.draftId }}</td>
                            ...
                            <td class="whitespace-nowrap" label="Views">{{ val.views }}</td>
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">Products 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">Products 수정</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                        <div>
                            <Number label="DraftId" v-model="selectedRow.draftId" :editMode="true"/>
                            ...
                            <Number label="Views" v-model="selectedRow.views" :editMode="true"/>
...
export default {
    ...
    methods:{
        async trackView(params){
            try{
                var path = "trackView".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','TrackView 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.trackViewDialog = false
            }catch(e){
                console.log(e)
            }
        },
